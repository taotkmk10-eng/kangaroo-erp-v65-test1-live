import{i as e}from"./realtimeImpactRegistry-uvlr93CK.js";import{c as t,n,o as r,r as i,s as a,t as o}from"./screenStack-BTwISuHL.js";import{t as s}from"./runtimeCore-t2AnId75.js";import{t as c}from"./confirmDialog-CLOcUN20.js";import{t as l}from"./toast-DvZC7wAk.js";function u(e){return e&&typeof e==`object`?e:{}}async function d({search:e=``,includeInactive:t=!0,limit:n=300}={}){return r(`rpc_get_nhan_vien_directory`,{p_search:String(e??``).trim()||null,p_limit:Number(n)||300,p_include_inactive:!!t},`Không tải được danh sách nhân viên`)}async function ee(e){return r(`rpc_tao_nhan_vien`,{p_request_key:i(`nhan-vien-create`),p_payload:u(e)},`Không tạo được nhân viên`)}async function te(e){return r(`rpc_cap_nhat_ho_so_nhan_vien`,{p_request_key:i(`nhan-vien-profile`),p_payload:u(e)},`Không cập nhật được nhân viên`)}async function ne(e){return r(`rpc_cap_nhat_phan_quyen_nhan_vien`,{p_request_key:i(`nhan-vien-permission`),p_payload:u(e)},`Không cập nhật được phân quyền`)}async function re(e){return r(`rpc_reset_mat_khau_nhan_vien`,{p_request_key:i(`nhan-vien-password`),p_payload:u(e)},`Không đổi được mật khẩu`)}async function ie(e){return r(`rpc_khoa_mo_nhan_vien`,{p_request_key:i(`nhan-vien-state`),p_payload:u(e)},`Không đổi được trạng thái nhân viên`)}var f=Object.freeze([{key:`MODULE_NHAN_VIEN_VIEW`,moduleId:`nhan_vien`,label:`Nhân viên`,description:`Luôn hiển thị để nhân viên xem hồ sơ và đổi mật khẩu của chính mình.`,required:!0},{key:`MODULE_TAO_DON_VIEW`,moduleId:`tao_don`,label:`Tạo đơn`,description:`Hiển thị module Tạo đơn trên Trang chủ.`},{key:`MODULE_GIAO_HANG_VIEW`,moduleId:`giao_hang`,label:`Giao hàng`,description:`Hiển thị module Giao hàng trên Trang chủ.`},{key:`MODULE_HOA_DON_VIEW`,moduleId:`hoa_don`,label:`Hóa đơn`,description:`Hiển thị module Hóa đơn trên Trang chủ.`},{key:`MODULE_DOI_SOAT_VIEW`,moduleId:`doi_soat`,label:`Đối soát`,description:`Hiển thị module Đối soát trên Trang chủ.`},{key:`MODULE_HAU_MAI_VIEW`,moduleId:`dich_vu`,label:`Dịch vụ`,description:`Hiển thị module Dịch vụ trên Trang chủ.`}]),p=new Set(f.map(e=>e.key));function m(e,t){e.dispatchEvent(new CustomEvent(`kangaroo:page-chrome`,{bubbles:!0,detail:t}))}function h(e,t=``){return String(e??``).trim()||t}function g(e){return h(e)||null}function _(e){switch(String(e??``).toUpperCase()){case`ACTIVE`:return`Đang hoạt động`;case`LOCKED`:return`Đã khóa`;case`DELETED`:return`Đã xóa`;default:return h(e,`Không xác định`)}}function v(e){return String(e??``).toUpperCase()===`ACTIVE`?`online`:`offline`}function y(e){let t=Array.isArray(e?.permission_keys)?e.permission_keys:[],n=e?.permissions??{},r=new Set([...t,...Array.isArray(n.modules)?n.modules:[],...Array.isArray(n.resources)?n.resources:[],...Array.isArray(n.fields)?n.fields:[],...Array.isArray(n.actions)?n.actions:[],...Array.isArray(n.special)?n.special:[]].map(String));return r.add(`MODULE_NHAN_VIEN_VIEW`),r}function b(e){let t=[e?.message,e?.details,e?.hint].map(e=>String(e??``).trim()).filter(Boolean).join(` `);for(let[e,n]of[[`EMPLOYEE_LOGIN_EXISTS`,`Tên đăng nhập đã tồn tại.`],[`EMPLOYEE_LOGIN_REQUIRED`,`Vui lòng nhập tên đăng nhập.`],[`EMPLOYEE_PASSWORD_REQUIRED`,`Vui lòng nhập mật khẩu.`],[`EMPLOYEE_NAME_REQUIRED`,`Vui lòng nhập tên hiển thị.`],[`EMPLOYEE_NOT_FOUND`,`Không tìm thấy nhân viên.`],[`RESET_OTHER_PASSWORD_FORBIDDEN`,`Bạn không có quyền reset mật khẩu nhân viên khác.`],[`EMPLOYEE_PROFILE_UPDATE_FORBIDDEN`,`Bạn không có quyền sửa hồ sơ nhân viên này.`],[`SELF_STATE_CHANGE_FORBIDDEN`,`Không thể tự khóa tài khoản của chính mình.`],[`EMPLOYEE_PROTECTED`,`Tài khoản hệ thống này không thể bị khóa.`],[`ROW_VERSION_CONFLICT`,`Dữ liệu đã thay đổi. Vui lòng tải lại rồi thao tác lại.`]])if(t.includes(e))return n;let n=t.match(/"message"\s*:\s*"([^"]+)"/i);return n?.[1]?n[1]:t||`Không thực hiện được thao tác.`}function x({name:e,label:n,value:r=``,type:i=`text`,required:o=!1,disabled:s=!1,autocomplete:c=`off`,textarea:l=!1,placeholder:u=``}){let d=`
  ...
  `;return`
    <label class="nhan-vien-field">
      ...
      ${l?`<textarea ${d}>${t(r)}</textarea>`:`<input
            type="${a(i)}"
            value="${a(r)}"
            ${d}
          />`}
  ...
  `}function ae(e){let n=h(e.trang_thai).toUpperCase();return`
    <button
      class="
        nhan-vien-card
      ...
      "
      type="button"
      data-employee-id="${a(e.id_nhan_vien)}"
    >
  ...
  `}function oe(e){return[[`Mã nhân viên`,e.ma_nhan_vien],[`Tên hiển thị`,e.ten_nhan_vien],[`Tên đăng nhập`,e.ten_dang_nhap],[`Số điện thoại`,e.so_dien_thoai],[`Email`,e.email],[`Địa chỉ`,e.dia_chi],[`Trạng thái`,_(e.trang_thai)]].filter(([,e])=>h(e)).map(([e,n])=>`
      ...
      `,r.querySelector(`[data-nhan-vien-list-root]`))}function X(){m(r,{headerMode:`search`,placeholder:`Tìm tên / mã / tài khoản...`,searchValue:P.snapshot().draft,onSearchInput(e){P.input(e)},onSearch(e){return P.submit(e,{reason:`submit`})},rightActions:[_.canManage?{key:`add`,icon:`+`,label:`Thêm nhân viên`,onAction:()=>u.open(`create`,Q)}:null,{key:`refresh`,icon:`↻`,label:`Tải lại`,onAction:()=>$({reason:`refresh`})}].filter(Boolean),bottomActions:_.canManage?[{key:`NHAN_VIEN_THEM`,label:`Thêm nhân viên`,variant:`primary`,onClick:()=>u.open(`create`,Q)}]:[]});let e=_.employees,t=fe(),n=t.querySelector(`[data-nhan-vien-list-count]`),i=t.querySelector(`[data-nhan-vien-list-error]`),a=t.querySelector(`[data-nhan-vien-list-phase]`),o=t.querySelector(`[data-nhan-vien-list]`);n.textContent=`${e.length} tài khoản`,i.hidden=!_.directoryError,i.textContent=_.directoryError,a.hidden=_.directoryPhase!==`refreshing`,o.innerHTML=e.length?e.map(ae).join(``):_.directoryPhase===`loading`?S(`Đang tải danh sách...`):_.directoryError?S(`Không tải được danh sách nhân viên.`):S(`Chưa có nhân viên phù hợp.`)}function Z(){let e=q();if(!e){u.back();return}m(r,{headerMode:`title`,title:`Chi tiết nhân viên`,onBack:()=>u.back(),showRightAction:!0,rightIcon:`↻`,rightLabel:`Tải lại`,onRightAction:()=>$({keepScreen:!0,reason:`detail-refresh`}),bottomActions:Y(e)});let n=y(e),i=f.filter(e=>n.has(e.key));r.innerHTML=`
      ...
      `}function Q(){m(r,{headerMode:`title`,title:`Thêm nhân viên`,onBack:()=>u.back(),showRightAction:!1,bottomActions:[{key:`create`,label:`Tạo nhân viên`,variant:`primary`,formAction:`submit`,disabled:_.busy}]}),r.innerHTML=`
          ...
          >
            <article class="nhan-vien-block">
              ...
              <div class="nhan-vien-permission-list">
                ${f.map(e=>{let r=e.required===!0||n.has(e.key);return`
                      <label
                        class="
